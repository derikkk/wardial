...
def enumerate_ips(start_ip, n):
    ...
    Ensuring that the return type is a generator
    is a proxy for testing for space efficiency of the function.
...
def netmask_to_ips(netmask):
    '''
    A netmask is a convenient shorthand for describing a ran
    ge of consecutive ip addresses.
    For details on the format, see: <https
    ://www.hacksplaining.com/glossary/netmasks>
...
async def is_server_at_host(session, host, schema='http'):
    ...
    The `session` variable is assumed to be a prop
    erly initialized `aiohttp.ClientSession` object.
    '''
    url = schema + '://' + host
    try:
...
async def _wardial_async(hosts, max_connections=500,
                         timeout=10, schema='http'):
    '''
    ...
    NOTE:
    Testing IO functions is made extra hard b
    ecause they rely on IO performing correctly.
    The tests above won't work if the google
    .com or microsoft.com webpages go down.

    NOTE:
    This is a helper function for the wardial function.
    In python, functions prefixed with an unders
    core are intended to be thought of as "private" or "internal" functions,
    ...
    async with aiohttp.ClientSession(
        ...
        # FIXME (Task 2):
        # The following code is "correct" in the se
        # nse that it gets the right results.
        # The problem is that it is not concurrent.
        # Modify the code to use the `asyncio.gathe
        # r` function to enable concurrency.
...
def wardial(hosts, **kwargs):
    '''
    Filter the `hosts` input to keep only those h
    osts with webservers running.
    ...
    # You will have to do some post-
    # processing of the results of this function to convert the output.
    host = list(hosts)
    loop = asyncio.new_event_loop()
